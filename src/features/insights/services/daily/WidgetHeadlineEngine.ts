...
export function computeWidgetHeadline(data: DailyInsightData): WidgetHeadlineData {
  if (__DEV__) console.log(`[LLM:Headline] computeWidgetHeadline() — meals=${data.todayMealCount}, cal=${data.todayCalories}/${data.calorieTarget} (${data.caloriePercent}%), protein=${data.proteinPercent}%, water=${data.waterPercent}%, streak=${data.loggingStreak}`);
  const now = Date.now();
  ...
  if (__DEV__) console.log('[LLM:Headline] → Rule 8 (standard progress)');
  return {
