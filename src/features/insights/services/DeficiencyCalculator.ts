...
export function calculateDeficiencies(input: DeficiencyCalculatorInput): DeficiencyResult {
  ...
  if (__DEV__) console.log(`[LLM:Deficiency] calculateDeficiencies() — daysUsingApp=${daysUsingApp}, daysSinceLastLog=${daysSinceLastLog}, nutrientDays=${dailyNutrientData.length}, dismissed=${dismissedAlerts.size}`);

  // Check minimum requirements
  if (daysUsingApp < ALERT_REQUIREMENTS.minDaysUsingApp) {
    if (__DEV__) console.log(`[LLM:Deficiency] → skipped: daysUsingApp ${daysUsingApp} < ${ALERT_REQUIREMENTS.minDaysUsingApp}`);
    return { checks: [], hasAlerts: false, alertCount: 0 };
  }

  if (daysSinceLastLog > ALERT_REQUIREMENTS.maxDaysSinceLastLog) {
    if (__DEV__) console.log(`[LLM:Deficiency] → skipped: daysSinceLastLog ${daysSinceLastLog} > ${ALERT_REQUIREMENTS.maxDaysSinceLastLog}`);
    return { checks: [], hasAlerts: false, alertCount: 0 };
  }

  const daysWithData = dailyNutrientData.filter((d) => d.hasData).length;
  if (daysWithData < ALERT_REQUIREMENTS.minDaysWithData) {
    if (__DEV__) console.log(`[LLM:Deficiency] → skipped: daysWithData ${daysWithData} < ${ALERT_REQUIREMENTS.minDaysWithData}`);
    return { checks: [], hasAlerts: false, alertCount: 0 };
  }
  ...
  for (const nutrientConfig of alertableNutrients) {
    ...
    if (__DEV__) console.log(`[LLM:Deficiency] ${name} (${id}): avg=${avgAmount.toFixed(1)}, rda=${rdaDefault}, %=${percentOfRDA.toFixed(1)}%, severity=${severity}`);

    // Check if alert is dismissed
    const alertId = `${id}_${severity}`;
    if (dismissedAlerts.has(alertId)) {
      if (__DEV__) console.log(`[LLM:Deficiency] ${id}_${severity} dismissed, skipping`);
      continue;
    }
  ...
  if (__DEV__) console.log(`[LLM:Deficiency] Result — ${checks.length} total deficiencies found, returning top ${topChecks.length}: [${topChecks.map((c) => `${c.nutrientId}(${c.severity})`).join(', ')}]`);
  return {
    checks: topChecks,
