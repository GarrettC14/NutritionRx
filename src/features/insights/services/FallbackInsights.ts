...
export function generateFallbackInsights(data: InsightInputData): Insight[] {
  if (__DEV__) console.log(`[LLM:Fallback] generateFallbackInsights() — cal=${data.todayCalories}, meals=${data.todayMealCount}, daysUsing=${data.daysUsingApp}`);
  const insights: Insight[] = [];

  if (data.todayCalories < 100 && data.todayMealCount === 0) {
    if (__DEV__) console.log('[LLM:Fallback] → early return: no meals logged today');
  ...
  if (data.daysUsingApp < 3) {
    if (__DEV__) console.log('[LLM:Fallback] → early return: new user (<3 days)');
  ...
  if (__DEV__) console.log(`[LLM:Fallback] Generated ${insights.length} fallback insights: [${insights.map(i => i.category).join(', ')}]`);
