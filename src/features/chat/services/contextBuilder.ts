...
export function buildSystemPrompt(context: ChatContext): string {
  const caloriePercent = context.todayLog.calorieTarget > 0
    ...
    : 0;
  if (__DEV__) console.log(`[LLM:Context] buildSystemPrompt() — cal=${context.todayLog.calories}/${context.todayLog.calorieTarget} (${caloriePercent}%), protein=${context.todayLog.protein}g/${context.todayLog.proteinTarget}g, weeklyAvgCal=${context.weeklyAverage.calories}, goal=${context.goals.primaryGoal}, recentFoods=${context.recentFoods.length}`);
...
export function detectSafetyTriggers(message: string): SafetyTrigger | null {
  if (__DEV__) console.log(`[LLM:Context] detectSafetyTriggers() — messageLength=${message.length}`);
